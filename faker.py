...
from constants import CONTENT_ROOT, DATA_NODES, MASTER_KEY, MASTER_TTL, NODES_TTL
from helpers import r_expires_on, r_get, r_set, r_master_etag, etag_master
...
def fake_all_nodes():
    ...
    for node_name in DATA_NODES:
        ...
        all_nodes_data[node_name] = fake_node_data(
            node_name, version=version, ttl=NODES_TTL
        )
    return all_nodes, all_nodes_data


def rebuild_all_fake_data(directory=True, conn=None, node=None):
    ...
    for node_name, node_data in all_nodes_data.items():
        if node is not None and node_name != node:
            continue
        ...
        if directory:
            ...
            with open(latest_node_file, "w") as f:
                print(
                    f'Writing node {node_name}@{node_data["version"]} to {latest_node_file}'
                )
                f.write(json.dumps(node_data, indent=4, sort_keys=True))
        if conn:
    ...
    if conn:
        ...
        r_master_etag(conn, etag_master(master_data["updated_on"]))
